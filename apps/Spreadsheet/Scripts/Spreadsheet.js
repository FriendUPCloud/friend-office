...
{
	Application.setApplicationName( i18n( 'i18n_spreadsheet' ) );
	...
	Application.menuItems = [
		...
		},
		{
			...
			items: [
				...
				{ name: i18n( 'i18n_new_sheet' ), command : 'insert_new_sheet' }
			]
		},
		{
			name: i18n( 'i18n_help' ),
	...
	else if( msg && msg.args && msg.args.indexOf( '.xlsx' ) > 0 )
	{
...
{
	Application.getApplicationsByName('Spreadsheet', function( rs ){
		
		if( rs.data && rs.data.length > 0 )
		{
			Application.otherInstanceCount = rs.data.length;
			Application.sendApplicationMessage( 'Spreadsheet', { 'command':'revealdocumentpath' }, function( cdprs ){
				//nothing to do
			} );
		}
		else
...
Application.receiveMessage = function( msg )
{
	...
	if( msg.message )
	{
		switch( msg.message.command )
		{
			...
			case 'showdocumentpath':
				if( msg.message.path == Application.documentPath )
				{
					Application.sendApplicationMessage( 'Spreadsheet', { 'command':'setwindowactive','path': Application.documentPath }, function( cdprs ){
						Application.quit();
					} );
				}
				else
			...
			case 'revealdocumentpath':
				...
				{
					Application.sendApplicationMessage( ( msg.source ? msg.source : 'Spreadsheet'  ), { 'command':'showdocumentpath','path': Application.documentPath }, function( cdprs ){
						//nothing more to do
					} );
				}
				break;
	...
	switch( msg.command )
	{
		case 'about':
		{
			...
			this.abw = new View( {
				title: 'About Friend Office Spreadsheet',
				width: 500,
				height: 500
			} );
...
Application.openFile = function( filePath, closeCurrentApp = false )
{
	...
	for(i = 0; i < Application.validFiles.length; i++)
	{
		if( filePath.indexOf('.' + Application.validFiles[ i ]) > 1 )
		{
			...
			Notify({'title':i18n('i18n_spreadsheet	'),'text':i18n('i18n_opening') + ' ' + filePath});
			if( closeCurrentApp ) { Application.editState = 'AVOID_THE_POPUP :)'; Application.quit() };
			return;
		}
	}
	Notify({'title':i18n('i18n_spreadsheet'),'text':i18n('i18n_invalid_filetype')});
...
Application.loadFileIntoEditor = function( fileItem, fileInfo, mode )
{
	...
	if( !fileItem )
	{
		var f = new File( 'Progdir:Templates/Spreadsheet.html' );
		f.onLoad = function( data )
		{
			// Add the file and set the new file
			Application.currentFile = { fileItem: false };
			Application.documentView.setFlag( 'title', 'New spreadsheet' );
			...
			Application.documentView.setContent( data, function()
			{
				...
				Application.documentView.sendMessage( { 'command': 'setpath', 'path': 'newsheet' } );
				...
				Application.documentView.sendMessage( { 'command': 'setfilename', 'filename': 'New_spreadsheet.xlsx', 'newfile': true } );
				...
				Application.currentFilename = 'New_spreadsheet.xlsx';
