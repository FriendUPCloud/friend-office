...
{
	Application.setApplicationName( i18n( 'i18n_presentation' ) );
	...
	Application.menuItems = [
		...
		},
		{
			...
			items: [
				...
				{ name: i18n( 'i18n_insert_slide' ), command : 'insert_slide' }
			]
		},
		{
			name: i18n( 'i18n_help' ),
	...
	// TODO: Support odp and ppt
	else if( msg && msg.args && msg.args.indexOf( '.pptx' ) > 0 )
	{
...
{
	Application.getApplicationsByName('Presentation', function( rs ){
		
		if( rs.data && rs.data.length > 0 )
		{
			Application.otherInstanceCount = rs.data.length;
			Application.sendApplicationMessage( 'Presentation', { 'command':'revealdocumentpath' }, function( cdprs ){
				//nothing to do
			} );
		}
		else
	...
	if( msg.message )
	{
		switch( msg.message.command )
		{
			...
			case 'showdocumentpath':
				if( msg.message.path == Application.documentPath )
				{
					Application.sendApplicationMessage( 'Presentation', { 'command':'setwindowactive','path': Application.documentPath }, function( cdprs ){
						Application.quit();
					} );
				}
				else
			...
			case 'revealdocumentpath':
				...
				{
					Application.sendApplicationMessage( ( msg.source ? msg.source : 'Presentation'  ), { 'command':'showdocumentpath','path': Application.documentPath }, function( cdprs ){
						//nothing more to do
					} );
				}
				break;
...
Application.openFile = function( filePath, closeCurrentApp = false )
{
	...
	for(i = 0; i < Application.validFiles.length; i++)
	{
		if( filePath.indexOf('.' + Application.validFiles[ i ]) > 1 )
		{
			...
			Notify({'title':i18n('i18n_presentation'),'text':i18n('i18n_opening') + ' ' + filePath});
			if( closeCurrentApp ) { Application.editState = 'AVOID_THE_POPUP :)'; Application.quit() };
			return;
		}
	}
	Notify({'title':i18n('i18n_presentation'),'text':i18n('i18n_invalid_filetype')});
...
Application.lockCreateInfoFile = function( fileItem, fileinfo, forcemode = false )
{
	...
	m.onExecuted = function( e, d )
	{
		...
		{
			Application.tryAgain( 'Could not set lock file. Please download your presentation instead.', function()
			{
				Application.loadFileIntoEditor( Application.fileItem, Application.fileInfo, 'view' );
				Application.documentView.setMenuItems( Application.editOnlyMenuItems );
				Application.documentView.setQuickMenu( Application.quickMenuEditOnly );
...
Application.tryAgain = function( errmsg, callback )
{
	...
	if( Application.retries > 1 )
	{
		...
		{
			Notify( {'title':i18n('i18n_error'),'text': i18n( errmsg ? errmsg : 'An error occurred opening your presentation.' ) } );
			Application.quit();
			return;
		}
	}
...
Application.loadFileIntoEditor = function( fileItem, fileInfo, mode )
{
	...
	if( !fileItem )
	{
		var f = new File( 'Progdir:Templates/Presentation.html' );
		f.onLoad = function( data )
		{
			// Add the file and set the new file
			Application.currentFile = { fileItem: false };
			Application.documentView.setFlag( 'title', 'New presentation' );
			...
			Application.documentView.setContent( data, function()
			{
				...
				Application.documentView.sendMessage( { 'command': 'setpath', 'path': 'newpresentation' } );
				...
				Application.documentView.sendMessage( { 'command': 'setfilename', 'filename': 'New_presentation.pptx', 'newfile': true } );
				...
				Application.currentFilename = 'New_presentation.pptx';
